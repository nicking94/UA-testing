...
import { CustomerFilters, customersApi } from "@/app/lib/api/customers";
...
const PresupuestosPage = () => {
  ...
  useEffect(() => {
    const loadBudgets = async () => {
      const allBudgets = (await fetchBudgetsApi()) as Budget[] || [];
    ...
    loadBudgets();
  }, [rubro, searchQuery, fetchBudgetsApi]);
  ...
  const handleConvertToSale = async (paymentMethods: PaymentSplit[]) => {
    ...
    try {
      ...
      await updateBudget(budgetToConvert.id, {
  ...
  const handleAddBudget = async () => {
    ...
    try {
      ...
      if (!customerId && newBudget.customerName) {
      ...
      const budgetToAdd: Omit<Budget, 'id'> = {
        date: newBudget.date,
        ...
        total: calculateTotalAndRemaining(newBudget.items, newBudget.deposit)
          .total,
        deposit: newBudget.deposit || "0",
        remaining: calculateTotalAndRemaining(newBudget.items, newBudget.deposit)
          .remaining,
        expirationDate: newBudget.expirationDate || undefined,
        notes: newBudget.notes || undefined,
        status: newBudget.status || "pendiente",
        rubro: rubro === "Todos los rubros" ? "comercio" : rubro,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      };
      const createdBudget = await addBudget(budgetToAdd);
      ...
      const allBudgets = await fetchBudgetsApi();
  ...
  const handleUpdateBudget = async () => {
    ...
    try {
      ...
      const updatedBudget = await updateBudget(editingBudget.id, updatedBudgetData);
      setBudgets(
