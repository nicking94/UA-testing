...
const useCreditSales = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [allSales, allPayments, allBackendCustomers] = await Promise.all([
        salesApi.getAll(),
        paymentsApi.getAll(),
        customersApi.getAll(),
      ]);
      const sales = allSales.filter(
        (sale: Sale) =>
          sale.credit === true && sale.creditType === "cuenta_corriente",
      ) as CreditSale[];
      const formattedCustomers = (
        allBackendCustomers as unknown as (Customer & { sales?: Sale[] })[]
      ).map((c) => ({
        ...c,
      ...
      return { sales, payments: allPayments, customers: formattedCustomers };
    } catch (error) {
        ...
        >
          ...
          <CardContent>
            ...
            {isFirstGreater(
              ...
              <Typography
                variant="body2"
                sx={{ mt: 1, color: "warning.main", fontWeight: "medium" }}
              >
...
const CuentasCorrientesPage = () => {
  ...
  const handleMarkCheckAsPaid = useCallback(
    async (checkId: number) => {
      try {
        await paymentsApi.update(checkId, { checkStatus: "cobrado" });
        const updatedData = await fetchData();
        if (isChequesModalOpen && currentCustomerInfo) {
          const allCheques = updatedData.payments.filter(
            (p) => p.method === "CHEQUE",
          );
          const validCheques = [];
          for (const cheque of allCheques) {
            const sale = updatedData.sales.find((s) => s.id === cheque.saleId);
            if (
              sale &&
              sale.customerName.toLowerCase().trim() ===
                currentCustomerInfo.name.toLowerCase().trim()
            ) {
              validCheques.push({ cheque, sale });
            }
          }
          const chequesWithDetails = validCheques.map(({ cheque, sale }) => {
            const saleItems =
        ...
        showNotification(
          "Cheque marcado como cobrado correctamente",
          "success",
        );
      } catch (error) {
    ...
    [fetchData, isChequesModalOpen, currentCustomerInfo, showNotification],
  );

  const handleDeleteCheck = useCallback(
    async (checkId: number) => {
      try {
        await paymentsApi.delete(checkId);
        const updatedData = await fetchData();
        if (isChequesModalOpen && currentCustomerInfo) {
          const allCheques = updatedData.payments.filter(
            (p) => p.method === "CHEQUE",
        ...
        if (currentCustomerInfo) {
          const updatedCustomer = updatedData.customers.find(
            (c) => c.name === currentCustomerInfo.name,
          );
          if (updatedCustomer) {
            setCurrentCustomerInfo({
              name: updatedCustomer.name,
              balance: updatedCustomer.pendingBalance,
              sales: updatedData.sales.filter(
                (s) => s.customerName === updatedCustomer.name && !s.chequeInfo,
              ),
            });
          }
        }
  ...
  const handleDeleteCustomerCredits = useCallback(async () => {
    ...
    try {
      ...
      const salesToDelete = creditSales
        ...
        .map((sale) => sale.id);
      await Promise.all(salesToDelete.map((id) => salesApi.delete(id)));
      const paymentsToDelete = payments
        .filter((p) => salesToDelete.includes(p.saleId))
        .map((p) => p.id);
      await Promise.all(paymentsToDelete.map((id) => paymentsApi.delete(id)));
      setCreditSales((prev) =>
      ...
      for (const method of paymentMethods) {
        if (method.amount > 0) {
          await paymentsApi.create({
            ...
            customerId: String(currentCreditSale.customerId),
            customerName: currentCreditSale.customerName,
          });
        }
      }

      setPaymentMethods([{ method: "EFECTIVO", amount: 0 }]);
      const updatedData = await fetchData(); // Re-fetch all data
  ...
  const handleOpenChequesModal = useCallback(
    async (customerName: string) => {
      try {
        ...
        const allCheques = payments.filter((p) => p.method === "CHEQUE");
        ...
        if (customerCheques.length === 0) {
          ...
          const validCheques: { cheque: Payment; sale: CreditSale }[] = [];
          for (const cheque of allCheques) {
            const sale = creditSales.find((s) => s.id === cheque.saleId);
            if (
              sale &&
              sale.customerName.toLowerCase().trim() ===
                customerName.toLowerCase().trim()
          ...
          const chequesWithDetails = customerCheques.map((cheque) => {
            const sale = creditSales.find((s) => s.id === cheque.saleId);
            const saleItems: SaleItem[] =
              sale?.products?.map((product) => ({
                productId: product.id,
    ...
    [payments, creditSales, currentCustomerCheques, showNotification],
  );
