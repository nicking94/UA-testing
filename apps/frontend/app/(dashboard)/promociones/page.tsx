...
import { usePromotionsApi } from "@/app/hooks/usePromotionsApi";
...
const PromocionesPage = () => {
  ...
  const {
    fetchPromotions: fetchPromotionsApi,
    addPromotion,
    updatePromotion,
    deletePromotion,
  } = usePromotionsApi();
  ...
  const fetchPromotions = async () => {
    try {
      const storedPromotions = await fetchPromotionsApi();
      const filtered =
  ...
  const handleConfirmAddPromotion = async () => {
    ...
    try {
      if (editingPromotion && editingPromotion.id) {
        await updatePromotion(editingPromotion.id, newPromotion);
        showNotification("Promoción actualizada correctamente", "success");
      } else {
        await addPromotion(newPromotion as Omit<Promotion, 'id'>);
