...
import { expenseCategoriesApi } from "@/app/lib/api/expense-categories";
import { expensesApi } from "@/app/lib/api/expenses";
import { useExpensesApi } from "@/app/hooks/useExpensesApi";
...
const MovimientosPage = () => {
  ...
  const {
    fetchExpenses: fetchExpensesApi,
    addExpense,
    updateExpense,
    deleteExpense,
  } = useExpensesApi();
  ...
  const loadSuppliers = useCallback(async () => {
    try {
      const storedSuppliers = (await suppliersApi.getAll()) as Supplier[];
  ...
  const loadCategories = useCallback(async () => {
    try {
      const storedCategories = (await expenseCategoriesApi.getAll()) as ExpenseCategory[];
  ...
  const loadExpenses = useCallback(async () => {
    try {
      const storedExpenses = await fetchExpensesApi();
  ...
  }, [fetchExpensesApi, showNotification]);
  ...
  const handleAddCategory = useCallback(async () => {
    ...
    try {
      ...
      const createdCategory = (await expenseCategoriesApi.create(categoryToAdd)) as ExpenseCategory;
  ...
  const handleDeleteCategory = async (category: ExpenseCategory) => {
    try {
      ...
      const expensesWithCategoryCount = allExpenses.filter(exp => 
        exp.category === category.name && exp.rubro === rubro
      ).length;
      if (expensesWithCategoryCount > 0) {
        showNotification(
