...
import { useSuppliersApi } from "@/app/hooks/useSuppliersApi";
import { useProductsApi } from "@/app/hooks/useProductsApi";
import { ProductFilters } from "@/app/lib/api/products";
...
import { Product, Supplier, SupplierContact, SupplierProduct } from "@/app/lib/types/types";
...
const ProveedoresPage = () => {
  ...
  const {
    fetchSuppliers: fetchSuppliersApi,
    addSupplier,
    updateSupplier,
    deleteSupplier,
  } = useSuppliersApi();
  ...
  const openProductAssignmentModal = async (supplier: Supplier) => {
    ...
    try {
      ...
      const allProducts = await fetchProductsApi(filters);
      const assignedSupplierProducts = await supplierProductsApi.getBySupplier(supplier.id);
      const assignedProductIds = assignedSupplierProducts.map((sp: SupplierProduct) => sp.productId);
  ...
  const unassignProduct = async (product: Product) => {
    if (!selectedSupplierForProducts) return;
    try {
      await supplierProductsApi.delete(selectedSupplierForProducts.id, product.id);
  ...
  const fetchSupplierProductCounts = useCallback(
    async (currentSuppliers?: Supplier[]) => {
      ...
      try {
        ...
        for (const supplier of suppliersToUse) {
          const assignedSupplierProducts = await supplierProductsApi.getBySupplier(supplier.id);
          const productIds = assignedSupplierProducts.map((sp: SupplierProduct) => sp.productId);
    ...
    [rubro, suppliers, fetchProductsApi]
  );
  const fetchSuppliers = useCallback(async () => {
    try {
      const allSuppliers = await fetchSuppliersApi();
  ...
  }, [rubro, fetchSuppliersApi, showNotification]);
  useEffect(() => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setFilteredSuppliers(
        filteredSuppliers.filter((s) => s.id !== supplierToDelete.id)
      );
